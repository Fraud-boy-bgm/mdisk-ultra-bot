...
async def start(bot, message):
    await message.reply(
        ...
        "**I am your Personal MDisk Bot 🤗, Made by @Maddy_353💞 Send me a MDisk Post to see the Magic 😅**")
    
@bot.on_message(filters.text & filters.private)
async def Doodstream_uploader(bot, message):
    new_string = str(message.text)
    conv = await message.reply("Hi Welcome Maddy Bot✋")
...
async def Doodstream_uploader(bot, message):
    ...
    conv = await message.reply("Hi this is Maddy Bot✋")
    dele = conv["message_id"]
    try:
        Doodstream_link = await multi_Doodstream_up(new_string)
